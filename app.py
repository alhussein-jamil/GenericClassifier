...
from src.data import load_dataset, create_data_generators
from src.models import create_model, train_model
from src.app import launch_app
from src.utils import create_default_config, save_config, load_config, update_config
...
def parse_args():
    ...
    # Main operation mode
    parser.add_argument('--mode', type=str, choices=['train', 'app', 'both'], default='app',
                       help='Operation mode: train, app, or both (default: app)')
    
    # Dataset parameters
    parser.add_argument('--zip', type=str, help='Path to dataset zip file')
    parser.add_argument('--extract_dir', type=str, help='Directory to extract the dataset to')
    ...
    parser.add_argument('--img_size', type=int, nargs=2, help='Image size (height width)')
...
def get_config(args) -> Config:
    ...
    if config is None:
        if args.zip:
            ...
            from src.schemas import DataConfig, ModelConfig, AppConfig
