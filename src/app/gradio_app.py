...
from src.schemas import AppConfig, Model, Config, DataConfig, ModelConfig
from src.models import load_model_from_checkpoint, create_model, train_model
from src.data import preprocess_image, load_dataset, extract_zip_dataset, create_data_generators
...
def load_model_schema(checkpoint_dir: Path, model_name: str) -> Optional[Model]:
    ...
    try:
        ...
        logger.info(f"Loaded model schema: {model_schema.info.name}, {model_schema.info.num_classes} classes")
        return model_schema
...
def plot_confusion_matrix(model_schema: Model) -> np.ndarray:
    ...
    class_names = [model_schema.info.class_mapping.get(str(i), f"Class {i}") 
    ...
    try:
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                if not has_folders:
                    return "error", "Zip file must contain folders (one per class)", None, None
        ...
        return "success", f"Dataset uploaded with {len(class_dirs)} classes", zip_path, extract_dir
...
def train_model_from_config(config: Config) -> Tuple[str, str, Optional[str]]:
    ...
    try:
        ...
        logger.info(f"Training completed with accuracy: {model_schema.results.accuracy:.4f}")
        ...
        return "success", f"Model trained successfully with accuracy: {model_schema.results.accuracy:.4f}", config.model.model_name
...
def create_all_in_one_app() -> gr.Blocks:
    ...
    def train_from_ui(
        ...
        if model_schema:
            ...
            return message, history_plot, confusion_matrix_plot, True, gr.update(visible=True)
        else:
    ...
    def predict_image(image):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                predicted_class = class_mapping.get(predicted_class_id, f"Unknown ({predicted_class_id})")
    ...
    with gr.Blocks(title="All-in-One Image Classification") as app:
        ...
        gr.Markdown("Upload a dataset, train a model, and classify images - all in one place!")
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("Train a Model"):
                with gr.Row():
                    with gr.Column():
                        ...
                        gr.Markdown("Upload a zip file containing your dataset organized in folders (one folder per class)")
                        ...
                        model_type = gr.Dropdown(
                            ...
                            choices=["mobilenetv2", "resnet50", "densenet121", "efficientnetb0", "vgg16"],
                            value="mobilenetv2"
                        )
                        
                        with gr.Row():
                            img_height = gr.Number(label="Image Height", value=224, precision=0)
                            img_width = gr.Number(label="Image Width", value=224, precision=0)
                        ...
                        with gr.Row():
                            ...
                            val_split = gr.Slider(label="Validation Split", minimum=0.1, maximum=0.5, value=0.2, step=0.05)
                        
                        with gr.Row():
                            use_pretrained = gr.Checkbox(label="Use Pretrained Weights", value=True)
                            use_augmentation = gr.Checkbox(label="Use Data Augmentation", value=True)
                    ...
                    with gr.Column():
                        ...
                        # Confusion matrix
                        confusion_matrix = gr.Image(label="Confusion Matrix", visible=True)
                        
                        # Training success indicator (hidden)
                        training_success = gr.Checkbox(label="Training Success", value=False, visible=False)
            ...
            with gr.TabItem("Classify Images"):
                with gr.Row():
                    with gr.Column():
                        # Image upload
                        image_input = gr.Image(type="filepath", label="Upload an image to classify (drag & drop)")
...
def create_app(config: AppConfig, checkpoint_dir: Path, model_name: str) -> gr.Blocks:
    ...
    if config.examples_dir and Path(config.examples_dir).exists():
        examples = [str(f) for f in Path(config.examples_dir).glob("*.*") 
    ...
    with gr.Blocks(title=config.title) as app:
        ...
        with gr.Tabs():
            with gr.TabItem("Classify Images"):
                with gr.Row():
                    with gr.Column():
                        image_input = gr.Image(type="filepath", label="Upload an image (drag & drop)")
            ...
            with gr.TabItem("Model Information"):
                with gr.Row():
                    with gr.Column():
                        ...
                        gr.Markdown(f"- **Input Shape:** {model_schema.info.input_shape}")
                        gr.Markdown(f"- **Parameters:** {model_schema.info.params_count:,}")
                        gr.Markdown(f"- **Date Trained:** {model_schema.info.date_trained}")
                        
                        if model_schema.results:
                            gr.Markdown(f"- **Accuracy:** {model_schema.results.accuracy:.4f}")
                            gr.Markdown(f"- **F1 Score:** {model_schema.results.f1_score:.4f}")
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        if model_schema.results and model_schema.results.confusion_matrix:
