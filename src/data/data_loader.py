...
from src.schemas import ClassInfo, DataConfig, Dataset, DatasetMetadata, ImageInfo
...
def load_dataset(config: DataConfig) -> Dataset:
    ...
    class_mapping = {
        class_dir.name: idx for idx, class_dir in enumerate(sorted(class_dirs))
    }
    ...
    for class_dir in class_dirs:
        ...
        image_files = [
            f for f in class_dir.glob("**/*") if f.is_file() and is_valid_image(f)
        ]
        ...
        for img_path in image_files:
            all_images.append(
                ImageInfo(path=img_path, class_name=class_name, class_id=class_id)
            )
    ...
    logger.info(
        f"Split dataset: {len(train_images)} training samples, {len(val_images)} validation samples"
    )
    ...
    for class_name, count in class_counts.items():
        classes.append(
            ClassInfo(name=class_name, id=class_mapping[class_name], sample_count=count)
        )
