...
from src.schemas import ModelConfig, Dataset, Model, ModelInfo, TrainingHistory, ModelResults
...
def train_model(
    ...
    for epoch in range(config.epochs):
        ...
        if val_acc > best_val_accuracy:
            ...
            logger.info(f"Saved best model checkpoint with validation accuracy: {val_acc:.4f}")
    ...
    eval_results = evaluate_model(
        model,
        val_loader,
        dataset.metadata.inv_class_mapping,
        device
    )
...
def _get_optimizer(optimizer_name: str, parameters, learning_rate: float) -> torch.optim.Optimizer:
    """
    ...
    if optimizer_name.lower() in optimizer_mapping:
        if optimizer_name.lower() == 'sgd':
            return optimizer_mapping[optimizer_name.lower()](parameters, lr=learning_rate, momentum=0.9)
        else:
            return optimizer_mapping[optimizer_name.lower()](parameters, lr=learning_rate)
    else:
...
def evaluate_model(
    ...
    logger.info(f"Evaluation results: Accuracy={results.accuracy:.4f}, F1={results.f1_score:.4f}")
...
def load_model_from_checkpoint(checkpoint_path: Path) -> nn.Module:
    ...
    try:
        ...
        schema_path = checkpoint_path.parent / f"{checkpoint_path.stem.split('_best')[0]}_schema.json"
        
        if not schema_path.exists():
            logger.warning(f"Schema file not found: {schema_path}")
            # Try using just the model name
            schema_path = checkpoint_path.parent / f"{checkpoint_path.stem.replace('_best', '')}_schema.json"
        ...
        if schema_path.exists():
            ...
            input_shape = model_schema_dict.get('info', {}).get('input_shape', [224, 224, 3])
            ...
            logger.info(f"Successfully loaded model {model_name} with {num_classes} classes")
            return model
        else:
            logger.warning("Schema file not found, attempting to infer model architecture from checkpoint")
            ...
            if any('features' in key for key in state_dict.keys()):
                ...
                if any('mobilenet' in checkpoint_path.stem.lower() or 
                       'mobilenetv2' in checkpoint_path.stem.lower()):
                    model = models.mobilenet_v2()
                    # Adjust the classifier
                    last_channel = model.classifier[1].in_features
                    model.classifier[1] = nn.Linear(last_channel, 2)  # Default to binary classification
                ...
                logger.warning("Could not determine model architecture, using a simple model")
                model = nn.Sequential(
            ...
            try:
                ...
                for key in state_dict.keys():
                    if 'weight' in key:
                        ...
                        if len(weight.shape) == 4:  # Conv layer
                            ...
                            layers[f"{layer_idx}"] = nn.Conv2d(in_channels, out_channels, 
                                                              kernel_size=3, padding=1)
                            layer_idx += 1
                        elif len(weight.shape) == 2:  # Linear layer
                            out_features, in_features = weight.shape
                            layers[f"{layer_idx}"] = nn.Flatten() if layer_idx == 0 else nn.Identity()
                            layer_idx += 1
                            layers[f"{layer_idx}"] = nn.Linear(in_features, out_features)
                            layer_idx += 1
                ...
                if layers:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to load model with dynamic architecture: {e}")
