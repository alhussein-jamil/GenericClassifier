...
def create_model(config: ModelConfig, input_shape: Tuple[int, int, int]) -> nn.Module:
    ...
    logger.info(f"Model compiled with optimizer: {config.optimizer}, lr: {config.learning_rate}")
...
def _get_base_model(model_name: str, input_shape: Tuple[int, int, int], pretrained: bool) -> nn.Module:
    """
    ...
    if model_name.lower() in model_mapping:
        ...
        logger.warning(f"Model {model_name} not found in predefined models, using a custom CNN")
...
def _add_classification_layers(base_model: nn.Module, model_name: str, num_classes: int) -> nn.Module:
    """
    ...
    if isinstance(base_model, nn.Module) and not hasattr(base_model, 'fc') and not hasattr(base_model, 'classifier'):
    ...
    # For standard models, replace the classification head
        ...
        # For models with special classifier structure like EfficientNet
            ...
            logger.warning(f"Model structure not recognized for {model_name}, creating a custom wrapper")
            model = nn.Sequential(
                base_model,
